...
import { BSC_CHAIN_ID, chains, DEFAULT_CHAIN_ID, OPTIMISTIC_CHAIN_ID, POLYGON_CHAIN_ID, XDAI_CHAIN_ID } from "../smart-contract/chains_constants";
    ...
    try{
      ...
      if (window.ethereum) {
        web3.eth.getChainId().then((chainId) => {
          ...
          web3.utils.toHex(chainId) === web3.utils.toHex(XDAI_CHAIN_ID))  connectWallet();
        })
...
export const checkNetworkById = async (chainId) => {
  ...
  if(globalWeb3)
  {
    ...
    globalWeb3.utils.toHex(chainId) !== globalWeb3.utils.toHex(XDAI_CHAIN_ID)) 
    {
      // await changeNetwork();     
      return false; 
    }
