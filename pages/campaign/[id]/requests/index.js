...
const RequestRow = ({
  ...
  const [isApprovingEnable, setIsApprovingEnable] = useState(true);
    ...
    try {
      ...
      if(campaign)
      {
        await campaign.methods.finalizeRequest(id).send({
          from: connectedAccount, gas: 3000000
        });
      }
      else{
  ...
  return (
    ...
    >
      ...
      <Td isNumeric>
        {globalWeb3?.utils.fromWei(request.value, "ether")} {chains[connectedChainId?.toString()]?.nativeCurrency}
      </Td>
      <Td>
        <Link
          color="teal.500"
          href={`${chains[connectedChainId?.toString()]?.blockScanUrl}/address/${request.recipient}`}
          isExternal
        >
          {" "}
...
export default function Requests({
  ...
  useEffect(() => {
      const loadCampaign = async () => {           
        try {
          let campaignId = router.query.id;
          console.log("[[id].js] campaignId = ", campaignId);
          let campaign = createCampaignContract(campaignId);
          console.log("[[id].js] campaign = ", campaign);
          const summary = campaign? await campaign.methods.getSummary().call() : [];
          ...
          if(Number(summary[2])>0 && campaign)
          {
          ...
          <Flex flexDirection={{ base: "column", md: "row" }} py={4}>
            <Box py="4">
              <Text fontSize={"lg"} color={"teal.400"}>
                <ArrowBackIcon mr={2} />
                <div onClick={() => {router.push(`/campaign/${router.query.id}`);}}>
                  Back to Campaign
                </div>
              </Text>
            </Box>
            ...
            <Box py="4">
              ...
              >
                {" "}
                {chains[nativeCurrency?.toString()]?.nativeCurrency}
              </Text>             
            </Box>
          </Flex>
          {FundNotAvailable ? (
            ...
            <Flex flexDirection={{ base: "column", lg: "row" }} py={4}>
              ...
              <Box py="2">
                  <Button
                    ...
                    onClick={() => {router.push(`/campaign/${campaignId}/requests/new/`);}}
                  >
            ...
            >
              <SimpleGrid row spacing={2} align="center">
                ...
                <Button
                  ...
                  onClick={() => {router.push(`/campaign/${router.query.id}/requests/new`);}}
                >
                ...
                <Button
                  ...
                  onClick={() => {router.push(`/campaign/${router.query.id}`)}}
                >
                    Go to Campaign
                </Button>
              </SimpleGrid>
